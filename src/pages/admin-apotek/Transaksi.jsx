    ...
    <div className="max-w-7xl mx-auto space-y-6 sm:p-2 lg:p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Kelola pesanan dan resep obat pasien.</p>
        </div>
        ...
        {statuses.map(status => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
              activeTab === status 
                ? 'bg-[#9ccda5] text-emerald-900 shadow-sm' 
              ...
              <CardContent className="p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-slate-900 dark:text-white">{trx.id}</span>
                    <Badge variant={getBadgeVariant(trx.status)}>{trx.status}</Badge>
                  </div>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Pasien: <span className="font-medium text-slate-700 dark:text-slate-300">{trx.patient}</span></p>
                  <p className="text-slate-400 dark:text-slate-500 text-xs">{trx.date} • {trx.doctor}</p>
                </div>
                <Button variant="outline" className="w-full sm:w-auto gap-2 dark:border-slate-700 dark:text-white">
                  <Eye size={16} /> Detail
                </Button>
              </CardContent>
            </Card>
